  ...
  <div id="app" class="app-background">
    <div class="container">
      ...
      <div class="button-group">
        <button class="large-button" @click="goToOrder">进行中</button>
      </div>
      ...
      <div class="next-appointments">
        <h2>下一预约</h2>
        <ul>
          <template v-if="nextAppointments.length > 0">
  ...
  methods: {
    async fetchData() {
      try {
        ...
        const allAppointments = response.data; // 假设返回的数据是用户数组

        // 过滤出与当前日期相同且在当前时间之前的预约
        this.nextAppointments = allAppointments.filter(item => {
          const appointmentDate = item.date; // 预约日期
          
          const appointmentEndTime = item.endtime; // 预约结束时间
          return (
            appointmentDate === this.currentDate && // 日期相同
            appointmentEndTime > this.currentTime // 结束时间在当前时间之后
          );
        });
        ...
        // 设置当前会议标题（如果当前时间在某个会议的开始和结束时间之间）
        const currentMeeting = allAppointments.find(item => {
          const appointmentDate = item.date;
          const appointmentStartTime = item.starttime;
          const appointmentEndTime = item.endtime;

          const currentDateTime = new Date(`${this.currentDate}T${this.currentTime}`); // 当前时间对象
          const meetingStartDateTime = new Date(`${appointmentDate}T${appointmentStartTime}`); // 会议开始时间对象
          const meetingEndDateTime = new Date(`${appointmentDate}T${appointmentEndTime}`); // 会议结束时间对象

          return (
            appointmentDate === this.currentDate && // 日期相同
            currentDateTime >= meetingStartDateTime && // 当前时间在开始时间之后
            currentDateTime <= meetingEndDateTime // 当前时间在结束时间之前
          );
        });

        // 设置当前会议标题
        if (currentMeeting) {
          this.currentMeetingTitle = currentMeeting.title; // 取当前会议的标题
        } else {
          this.currentMeetingTitle = '没有会议'; // 如果没有会议
        }

        // 更新标题
        this.title = this.currentMeetingTitle;
      } catch (error) {
    ...
    goToOrder() {
      this.$router.push('/main/page1'); // 跳转到详情页面
    },
    showMeetingForm() {
